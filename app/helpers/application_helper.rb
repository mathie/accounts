module ApplicationHelper
  def topnav_link_to(name, path)
    content_tag(:li, class: current_section?(path) ? 'active' : nil) do
      link_to name, path
    end
  end

  # Really, really dumb way of figuring out which section we're in -- so,
  # which topbar nav to highlight. Probably wants extending at some point.
